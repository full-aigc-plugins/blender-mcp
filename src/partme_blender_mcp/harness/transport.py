...
class JsonLineServer:
    ...
    def start(self) -> Endpoint:
        ...
        if self.endpoint.kind == "tcp":
            ...
            self._thread = threading.Thread(target=self._serve_pipe, name="partme-blender-pipe", daemon=True)
            self._thread.start()
            return self.endpoint
        else:
            raise ValueError(f"unknown transport kind: {self.endpoint.kind}")
        self._thread = threading.Thread(target=self._server.serve_forever, name="partme-blender-transport", daemon=True)
        self._thread.start()
        return self.endpoint
