...
> **预发布提醒**：当前没有可下载的 Runtime Release。必须等待 [GitHub Releases](https://github.com/partme-ai/blender-mcp/releases/latest) 出现对应文件后再执行。

## 1. 下载

- 从 [Blender 官网](https://www.blender.org/download/)下载 Windows Installer。
- 下载 `partme-blender-mcp-addon-<version>.zip`。
- 下载 `partme-blender-mcp-runtime-<version>.zip` 和 `SHA256SUMS.txt`。

## 2. 校验 SHA-256

```powershell
Set-Location "$HOME\Downloads"
Get-FileHash .\partme-blender-mcp-addon-<version>.zip -Algorithm SHA256
Get-FileHash .\partme-blender-mcp-runtime-<version>.zip -Algorithm SHA256
```
...
2. 右上角菜单选择 **从磁盘安装…**。
3. 选择 `partme-blender-mcp-addon-<version>.zip`，不要解压。
