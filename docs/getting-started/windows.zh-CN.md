...
> **发布提醒**：`v0.7.0-rc.2` 是候选版本。Windows 安装包已构建，并须以 GitHub Windows CI 通过为发布依据。

## 1. 下载

- 从 [Blender 官网](https://www.blender.org/download/)下载 Windows Installer。
- 下载 `partme-blender-mcp-addon-0.7.0-rc.2.zip`。
- 下载 `partme-blender-mcp-runtime-0.7.0-rc.2.zip` 和 `SHA256SUMS.txt`。

## 2. 校验 SHA-256

```powershell
Set-Location "$HOME\Downloads"
Get-FileHash .\partme-blender-mcp-addon-0.7.0-rc.2.zip -Algorithm SHA256
Get-FileHash .\partme-blender-mcp-runtime-0.7.0-rc.2.zip -Algorithm SHA256
```
...
1. 下载 `partme-blender-mcp-windows-x64-0.7.0-rc.2.zip`。
...
```powershell
py -3.13 -m pip install .\partme-blender-mcp-windows-x64-0.7.0-rc.2.zip
```
...
2. 右上角菜单选择 **从磁盘安装…**。
3. 选择 `partme-blender-mcp-addon-0.7.0-rc.2.zip`，不要解压。
