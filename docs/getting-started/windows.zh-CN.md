...
> **预发布提醒**：`v0.4.0` 是正式版本。Windows 安装包已构建，但发布前必须等待 GitHub Windows CI 通过。

## 1. 下载

- 从 [Blender 官网](https://www.blender.org/download/)下载 Windows Installer。
- 下载 `partme-blender-mcp-addon-0.4.0.zip`。
- 下载 `partme-blender-mcp-runtime-0.4.0.zip` 和 `SHA256SUMS.txt`。

## 2. 校验 SHA-256

```powershell
Set-Location "$HOME\Downloads"
Get-FileHash .\partme-blender-mcp-addon-0.4.0.zip -Algorithm SHA256
Get-FileHash .\partme-blender-mcp-runtime-0.4.0.zip -Algorithm SHA256
```
...
1. 下载 `partme-blender-mcp-windows-x64-0.4.0.zip`。
...
```powershell
py -3.13 -m pip install .\partme-blender-mcp-windows-x64-0.4.0.zip
```
