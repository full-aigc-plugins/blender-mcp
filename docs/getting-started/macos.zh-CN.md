...
> **预发布提醒**：`v0.5.2` 是正式版本。只从 GitHub Release 下载，并在安装前核对 SHA-256。

## 1. 下载两个组件

PartMe Blender MCP 包含：

1. `partme-blender-mcp-runtime-0.5.2.zip`：由 MCP 客户端启动。
2. `partme-blender-mcp-addon-0.5.2.zip`：安装到 Blender。
...
```bash
cd "$HOME/Downloads"
shasum -a 256 partme-blender-mcp-addon-0.5.2.zip
shasum -a 256 partme-blender-mcp-runtime-0.5.2.zip
```
...
1. 下载 `partme-blender-mcp-macos-arm64-0.5.2.tar.gz`。
...
```bash
python3.13 -m pip install ./partme-blender-mcp-macos-arm64-0.5.2.tar.gz
```
...
`v0.5.2` 已通过源码 ZIP 的隔离 pip 安装与 MCP initialize 测试。
...
1. 选择 `partme-blender-mcp-addon-0.5.2.zip`，不要解压。
