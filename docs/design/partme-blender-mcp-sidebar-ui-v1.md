...
> 状态：V4.2 视觉基线与社区式紧凑行修订已确认；当前源码完成局部可见验证及功能回归，最终视觉、外部供应商、全平台与正式安装发布门禁仍未全部完成。证据见 `../verification/ui-function-acceptance-2026-09-20.md`。
> 初次定稿：2026-09-19
> 多传输修订：2026-09-19
> 适用范围：Blender `PartMe MCP` 侧栏
> 架构规格：[PartMe Blender MCP 供应商整合设计](../superpowers/specs/2026-09-19-provider-integration-design.md)

## 最新确认：社区式紧凑行（2026-09-20）

此修订优先于下文旧视觉稿中的供应商右侧开关及常驻状态行。

- 保留四个 Tab。素材和模型共用 `左侧小勾选框｜图标＋完整名称｜状态点｜可选小齿轮`，不为不同供应商维护独立布局分支。
- 正常或停用供应商只占一行；缺少配置、错误和任务进展才展开详情。配置使用小齿轮，禁止整宽「配置」「开／关」按钮。
- 本地素材库显示不可取消的已选框；缺配置或生成中保持原有启停锁定规则。
- HTTP / SSE 左侧小勾选框调用真实启停操作，下方独立显示运行状态，不能把勾选意图当作启动成功。
- 窄侧栏按 View2D 与界面缩放计算实际宽度；异常说明换行，不挤压名称。生成任务保留真实终止／停止等待语义。
- 实现验证与发布分开记录；临时加载及模拟进度不代表发布包或真实供应商生成验收。
...
Tab 绑定到`WindowManager`上的 EnumProperty，例如`partme_blender_ui_tab`，并使用原生`UILayout.row().prop(..., expand=True)`绘制。真实 Blender 5.2.1 已验证：`UILayout.prop_tabs_enum()`会在约`240 px`窄侧栏中折叠为只剩当前项，不能满足四个入口始终可见的验收要求。Tab 选择属于临时界面状态，不写入`.blend`和 Scene 自定义属性。四个枚举值为：
...
- 访问地址与`复制`按钮同行，地址占剩余空间；按钮必须预留两个汉字和内边距的宽度，不因固定 90:10 比例裁成“复”。按钮无附加图标；只复制公开端点，不复制令牌、密钥或其他凭证；
...
供应商可通过注册表 `metadata.uiOrder` 控制同类项的稳定显示顺序；未声明时按名称排在显式顺序之后。状态圆点使用 Add-on 随包发布的 Blender preview icon，固定映射绿色（就绪）、琥珀色（需配置/审批）、蓝色（处理中）、灰色（关闭）和红色（错误），同时始终保留文字状态。新增供应商只需注册相同协议字段，不修改四 Tab renderer。
...
- 使用 Blender 原生展开式 EnumProperty 按钮行；不得使用会在窄侧栏隐藏未选项的`prop_tabs_enum`；
- 固定顺序为`制作`、`素材`、`模型`、`接入`；
- 默认选中`制作`；
- 每次只绘制一个 Tab 的内容；
...
- `本地 MCP`标题下直接显示 stdio 状态，不再放按需启动说明；
...
| stdio 已就绪 | 绿色 | 状态行显示`已就绪`，无额外按需启动说明 |
| 远程传输已关闭 | 中性灰 | `已关闭` |
...
- [x] 使用 Blender 原生展开式 EnumProperty 按钮行实现`制作`、`素材`、`模型`、`接入`四个始终可见的 Tab；
- [x] 首次默认选中`制作`，`接入`不默认显示；
- [x] Tab 状态保存在`WindowManager`，不写入`.blend`和 Scene 自定义属性；
- [x] `执行设置`通过独立弹窗打开，并以`保存并应用`原子更新运行时策略；
- [x] 在`240 px`侧栏宽度下，权限字段标题全部居左且左边缘一致；
- [x] AI 模型卡片按名称、状态、进度/阶段、动作纵向排列，没有控件相互挤压；
- [x] 场景版本只在顶部总览出现一次，制作 Tab 的等待操作完整显示且不被遮挡；
- [x] `供应商可用 x / y`由注册表实时计算，其中`x = enabled && (ready || busy)`、`y = registered total`，不使用硬编码或含义不明的“供应商能力”；
- [x] 隐藏 Tab 出现待审批、生成完成/失败或接入错误时，总览显示可跳转提醒；
- [x] 界面层级、状态色和间距与确认后的 V4 视觉稿一致；
...
- [x] 非 loopback 监听必须鉴权，对公网或内网穿透暴露必须使用 HTTPS；
...
- [x] 配置缺失时供应商开关关闭且禁用，`配置`打开该供应商的定向安全表单；
- [x] 供应商密钥不在主面板、`.blend`、Scene 属性、状态快照、日志、回执或剪贴板中明文出现；
...
- [x] 在真实 Blender 中验证窄宽度、Tab 切换、默认 Tab、设置弹窗和全部关键状态；
- [x] 以本机 MCP Host 验证 stdio 就绪、活动客户端进入/退出和不可用原因恢复；
- [ ] 以同一局域网的第二台设备验证 Streamable HTTP 的鉴权、并发客户端、重连和关闭开关；
- [x] 以仍使用旧传输的客户端验证 SSE 兼容入口与独立关闭；
- [ ] 以平板兼容客户端或等价远程设备验证公开地址、HTTPS、审批和场景版本冲突反馈。

### 9.1 真实验收证据（2026-09-20）

- Blender `5.2.1 LTS` 可见运行：`561` device-pixel UI region（Retina 下约等效 `280 px`），默认`制作`，四 Tab 均可切换；正常、生成中 `68%`、密钥缺失、失败、审批及执行设置弹窗均人工可见核对；
- `addon_and_approval_smoke.py`：默认自动供应商操作无审批；累计预算 `5.00` 下首个 `3.00` 自动执行、第二个 `3.00` 进入 `AUTHORIZATION_REQUIRED`；
- `remote_transport_smoke.py`：stdio `ready`，Streamable HTTP 与 SSE 同时各有一个官方 SDK 客户端；单独关闭 HTTP 后 SSE 保持运行，再独立关闭 SSE；
- `generic_mcp_client_handshake.py`：官方协议 `2025-06-18`、`173` 个工具跨 `4` 页唯一列出，本地审批后同一请求重试成功；
- `provider_toggle_smoke.py`：社区贡献加载后共 `6` 个供应商，Poly Haven 偏好、运行时属性、注册表状态和界面状态同步切换。
- 2026-09-21 当前候选：官方 SDK stdio、Streamable HTTP、SSE 均为 `177` 个工具、`4` 页，schema 摘要一致；HTTP 同时 `2` 客户端、SSE 同时 `1` 客户端，断开归零、HTTP 重连与两传输独立关闭通过。loopback HTTP 与自签名 HTTPS 均验证 Bearer 缺失/错误返回 `401`。这证明本机协议与并发链路，不替代上方第二台物理设备和平板门禁。
# 模型启用交互补充（2026-09-20）

- 缺少凭证不禁用左侧勾选入口；点击后打开对应供应商配置，确认按钮为“保存并启用”。
- 配置完整后保存启用偏好并立即刷新；取消对话框不启用，不提交生成任务。
- 齿轮入口只保存配置，不隐式改变启停偏好；生成期间仍锁定启停。
- 配置就绪仅表示本地配置完整，不代表远端凭证已验证或获得付费授权。
- Hyper3D 配置先选择`客户端 OAuth（推荐）`或`API Key`。OAuth 模式显示 Codex/Claude Code、固定 MCP 地址、授权状态和`浏览器授权`；
  不显示 API Key 输入框，也不把 Token 写入 Blender。API Key 模式才显示平台与密码输入框。两种模式切换时保留另一模式的本机配置，
  但执行只能使用当前选中模式；OAuth 生成由客户端 Hyper3D MCP 完成，结果仍必须经过授权目录和事务导入。
