...
配置字段应进入 Add-on Preferences 或受保护的本地配置，不写入 `.blend`、Scene 自定义属性、公开 URL、日志或剪贴板。界面只允许复制公开端点，客户端凭证通过独立的安全配置流程交付。
...
- 配置入口打开对应供应商的定向安全表单。API Key、Secret 和 Token 不在主面板直接输入，不写入`.blend`、Scene 自定义属性、状态快照、日志、回执或剪贴板；
