...
- 不生成故事、剧本或跨镜头制片计划；
- 不静默或越权调用 Rodin、混元、Sketchfab 等外部供应商服务；
...
从 [v0.7.0-rc.2](https://github.com/full-aigc-plugins/blender-mcp/releases/tag/v0.7.0-rc.2) 下载：
...
`0.7.0-rc.2` 是当前候选版本。Harness 暂时保留 `codex-blender/v1`，使现有 `codex-blender-plugin` 能进行差分迁移；新公共身份、MCP Server ID 和 Add-on 均使用 PartMe。未经验证的客户端保持 `DOCUMENTED_NOT_RUN`。
