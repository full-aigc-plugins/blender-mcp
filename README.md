...
Download from [v0.5.3](https://github.com/full-aigc-plugins/blender-mcp/releases/tag/v0.5.3):

```text
partme-blender-mcp-addon-0.5.3.zip
partme-blender-mcp-runtime-0.5.3.zip
SHA256SUMS.txt
```
...
```bash
python -m pip install ./partme-blender-mcp-runtime-0.5.3.zip
...
3. Select `partme-blender-mcp-addon-0.5.3.zip` without extracting it.
...
`0.5.3` is the current release. The Harness keeps `codex-blender/v1` temporarily for differential migration from `codex-blender-plugin`; all public identity, MCP server ID, and Add-on surfaces use PartMe. Untested clients remain `DOCUMENTED_NOT_RUN`.
